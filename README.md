# 💻 Riachuelo - Ciberseguranca

🎯 Trilha prática de segurança com Linux, redes, virtualização e hacking ético. Conteúdo atualizado com IA, desafios e projetos reais para desenvolver habilidades ofensivas e defensivas em cenários que vão de varredura a mitigação de ataques.


## 1. 🗂️ Princípios e Aprendizagem Com Inteligência Artificial  



- Bem vindo à Experiência Riachuelo Cibersegurança


- A Era da IA: Machine Learning, LLMs, IA Generativa e Agentes


- Fundamentos de Modelos de Linguagem de Grande Escala


- Introdução à Engenharia de Prompts


- Técnicas de Engenharia de Prompt


- Desafios de Projetos: Crie Um Portfólio Vencedor


- **Desafio de projeto:** Acelere sua Aprendizagem com IA: Explore o Poder do NotebookLM

### 🚀 Explorando a Nuvem: Conceitos, Arquitetura e AWS

**Link para o projeto no NotebookLM:**  ***https://notebooklm.google.com/notebook/a9571037-963c-4b6b-89de-f41d1e755300***




## 2. 🗂️ O Essencial Sobre Linux e Máquinas Virtuais  


- Introdução ao Sistema Operacional Linux


- Instalando o Linux


- Acesso Remoto a Máquinas Linux


- Manipulando Arquivos no Linux


- Gerenciando Usuários no Linux


- **Desafio de código:** Treinando Comandos de Linux com Python




## 3. 🗂️ Fundamentos de Cibersegurança  

- Princípios da Cibersegurança


- Conceitos e Práticas de Sistemas Operacionais e Máquinas Virtuais


- Fundamentos de Redes de Computadores


- Introdução à Deep Web e Anonimato


- **Desafio de código:** Conceitos de Cibersegurança com Python




## 4. 🗂️ Hacking Ético: Vulnerabilidades e Testes de Invasão  


- Fundamentos de Testes de Invasão (Pentest)



- Introdução à Coleta e Análise de Segurança Cibernética



- Tópicos em Engenharia Social



- Conceitos e Técnicas de Varredura de Rede



- Princípios de Enumeração e Exploração de Vulnerabilidades


- **Desafio de projeto:** Simulando um Ataque de Brute Force de Senhas com Medusa e Kali Linux



## 5. 🗂️ Avançando na Análise e Proteção de Ataques Cibernéticos  


- Técnicas de Exploração de Vulnerabilidades



- Pós-Exploração em Sistemas Comprometidos



- Man in the Middle: Ataques e Mitigações


- **Desafio de projeto:** Simulando um Malware de Captura de Dados Simples em Python e Aprendendo a se Proteger

